...
import uk.bot_by.monobank4j.util.WebhookLocatorExpander;
...
public interface Personal {
	...
	List<StatementItem> getStatement(@Param(ACCOUNT) @NotNull String account,
	                                 @Param(value = FROM, expander = UnixTimeExpander.class) @NotNull Instant from,
	                                 @Param(value = TO, expander = UnixTimeExpander.class) @Nullable Instant to);
	...
	@Body(WEBHOOK_BODY)
	Response setWebhook(@Param(value = WEBHOOK_LOCATOR, expander = WebhookLocatorExpander.class) @Nullable URL webhookLocator);
